    ...
    Jconfirm.prototype = {
        ...
        _setButtons: function () {
			...
			if (this.cancelButton && $.trim(this.cancelButton) !== '') {
				if (this.buttonsReverse) {
					this.$cancelButton = $('<button type="button" class="btn">' + this.cancelButton + '</button>').prependTo(this.$btnc).addClass(this.cancelButtonClass);
				} else {
			...
			/*添加一个用于在按钮边上放其他元素，如checkbox*/
			if (this.buttonOther && $.trim(this.buttonOther) !== '') {
				$(this.buttonOther).prependTo(this.$btnc);
			}
